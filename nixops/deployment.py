    ...
    def activate_configs(
            ...
            try:
                ...
                if not dry_activate:
                    ...
                    if not m.is_okay_to_deploy():
                        ...
                        raise Exception(
                            "Failed to prepare {m.name} for deployment (deploy-prepare.target failed)"
                        )
                ...
                m.success("starting deploy-healthy.target")
                if not m.mark_deploy_healthy():
                    m.warn("refused to go to deploy-healthy.target")
                    raise Exception("{m.name} refused to go to deploy-healthy.target")
        ...
        def worker_deploy_completed(m: nixops.backends.MachineState) -> None:
            ...
            if not m.mark_deploy_complete():
                ...
                raise Exception("{m.name} refused to go to deploy-completed.target")
